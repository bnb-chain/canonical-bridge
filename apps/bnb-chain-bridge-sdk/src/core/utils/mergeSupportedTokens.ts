...
export interface MergeSupportedTokensFuncParams {
  ...
  stargate: {
    availableTokens: Set<string>;
    tokenPairs: StarGateTokenPair[];
  };
}

export function mergeSupportedTokens(params: MergeSupportedTokensFuncParams) {
  const { tokenOrder, cBridge, deBridge, stargate } = params;
  ...
  deBridge.tokenPairs.forEach((item) => {
    ...
    if (existingToken) {
      ...
      tokens.push(bridgeToken);
      existingMap.set(toUpperAddress(fromToken.address), bridgeToken);
    }
  });

  // stargate tokens
  stargate.tokenPairs.forEach((item) => {
    const { fromToken } = item;

    const existingToken = existingMap.get(toUpperAddress(fromToken.address));
    const isAvailable = stargate.availableTokens.has(fromToken.symbol);

    if (existingToken) {
      existingToken.rawData.stargate = isAvailable ? fromToken : undefined;
      existingToken.available.stargate = isAvailable;
    } else {
      const bridgeToken: BridgeToken = {
        ...
        isPegged: false,
        rawData: {
          stargate: isAvailable ? fromToken : undefined,
        },
        available: {

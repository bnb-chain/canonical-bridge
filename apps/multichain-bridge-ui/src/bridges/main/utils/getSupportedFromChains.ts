...
export function getSupportedFromChains(params: GetBridgeChainsParams) {
  const { rawData } = params;

  const cbridgeChainIdSet = getCBridgeChainIdSet(params);
  const deBridgeChainIdSet = getDeBridgeChainIdSet(params);

  const cbridgeRes = rawData.cbridge.chains.filter((item) => cbridgeChainIdSet.has(item.id));
  const deBridgetRes = rawData.debridge.chains.filter((item) =>

...
export type CBridgeTokenPair = {
  fromTokenAddress: string;
  toTokenAddress: string;
  fromToken: CBridgeToken;
  toToken: CBridgeToken;
  isPegged?: boolean;
  peggedPair?: CBridgePeggedPairConfig;
};
...
function getTokenConfigs(
  ...
  Object.entries(chain_token).forEach(([id, { token: tokens }]) => {
    ...
    const filteredTokens = tokens.filter((token) => {
      ...
      const isNativeToken = nativeTokenMap.get(chainId) === token.token.symbol;
...
function getRelatedMap(
  ...
  const relatedMap = new Map<number, Map<number, Map<string, CBridgeTokenPair>>>();
  ...
  chains.forEach((fromChain) => {
    chains.forEach((toChain) => {
      if (fromChain.id !== toChain.id) {
        ...
        const relatedTokenMap = new Map<string, CBridgeTokenPair>();
        ...
        if (relatedTokenMap.size > 0) {
          if (!relatedMap.has(fromChain.id)) {
            relatedMap.set(fromChain.id, new Map<number, Map<string, CBridgeTokenPair>>());
          }
  ...
  const addPeggedTokenPair = (
    ...
    if (!relatedMap.get(fromChainId)?.get(toChainId)?.get(fromToken.token.symbol)) {
      if (!relatedMap.has(fromChainId)) {
        relatedMap.set(fromChainId, new Map<number, Map<string, CBridgeTokenPair>>());
      }

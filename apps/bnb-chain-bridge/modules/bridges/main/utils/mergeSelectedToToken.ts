...
export function mergeSelectedToToken(params: MergeToTokenFuncParams) {
  ...
  if (!cBridgeToken && !deBridgeToken) {
    return;
  }

  const name = (cBridgeToken?.name ?? deBridgeToken?.name)!;
  const symbol = (cBridgeToken?.token.symbol ?? deBridgeToken?.symbol)!;
  const decimal = (cBridgeToken?.token.decimal ?? deBridgeToken?.decimals)!;
  const address = (cBridgeToken?.token.address ?? deBridgeToken?.address)!;

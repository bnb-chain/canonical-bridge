...
export const useGetDeBridgeFees = () => {
  ...
  const { getDeBridgeTokenByAddress } = useBridgeConfigs();
    ...
    (async () => {
      try {
        const amount = parseUnits(
          sendValue,
          selectedToken?.rawData?.deBridge?.decimals ?? (18 as number),
        );
        if (
  ...
  const marketMakerFee = useMemo(() => {
    ...
    if (srcChainInTokenIn) {
      ...
      const token = getDeBridgeTokenByAddress({
        chainId: chain,
        address: tokenIn,
      });
    ...
    return marketFeeStr;
  }, [estimatedAmount?.deBridge, getDeBridgeTokenByAddress]);
